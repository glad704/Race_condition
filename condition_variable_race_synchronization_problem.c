/*************************************************************************
 * FILE         : condition_variable_race/synchronization_problem().c
 * DESCRIPTION  : This program demonstrates a condition variable race
...
void* sub1(void* t)
{
         ...
         * is reached before this routine is run by the waiting thread, the loop
         * will be skipped to prevent pthread_cond_wait from never returning
         * and that this thread's work is now done within the mutex lock of 
	 * count*/
...
void* sub2(void* t)
{
        ...
        for(i=0;i<ITERATIONS;i++)
        {
                 ...
                 * condition variable is reached. Note that this occurs while
                 * mutex is locked.*/
		if(count==THRESHOLD)
                {
                        printf("sun2: thread=%ld Threshold reached. count=%d.",tid,count);
...
int main(int argc,char *argv[])
{
        ...
        /* For portability, excplicitly create threads in a joinable state*/
